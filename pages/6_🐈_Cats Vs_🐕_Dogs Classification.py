...
def load_model_catsdogs():
	...
	model = tf.keras.models.load_model('pages/Model/dogs_and_cats_VGGclassifier.h5')
...
def gift():
		...
		file_ = open("pages/Assets/catdog.gif", "rb")
		contents = file_.read()
...
def main():
	st.title("NHẬN DIỆN CHÓ VÀ MÈO SỬ DỤNG CNN")

	analysis = st.sidebar.selectbox("Index", ["Introduction", "Cats v/s Dogs"])
	if analysis == "Introduction":

		st.header("Cats vs Dogs")
		st.image('pages/Assets/catdog1.jpeg', width= 700, use_column_width=True)
		...
		st.subheader("Type: Dogs V/S Cats Classifier")
